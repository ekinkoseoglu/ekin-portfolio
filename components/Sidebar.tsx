...
import { Home, Briefcase, User, Mail, Code2, Menu, X, Github, Linkedin } from 'lucide-react';
...
const Sidebar: React.FC = () => {
  ...
  const scrollToSection = (id: string) => {
    ...
    if (element && mainContent) {
      // Add sliding class to trigger the "slide effect" on the page
      mainContent.classList.remove('nav-sliding');
      // Trigger reflow
      void mainContent.offsetWidth; 
      mainContent.classList.add('nav-sliding');
  ...
  return (
    <>
      <div className={`md:hidden fixed top-6 right-6 z-50 transition-all duration-700 delay-300 transform ${mounted ? 'translate-y-0 opacity-100' : '-translate-y-20 opacity-0'}`}>
        <button
            ...
            return (
              ...
              >
                <div className={`
                    ...
                    ${isActive ? 'bg-brand-600 dark:bg-brand-400 opacity-100' : 'bg-transparent opacity-0 group-hover/item:opacity-30 group-hover/item:bg-slate-400'}
                `}></div>

                <div className="w-24 h-full flex items-center justify-center flex-shrink-0">
                    <div className={`
                        p-2.5 rounded-xl transition-all duration-300
                        ${isActive ? 'bg-brand-50 dark:bg-brand-900/20 shadow-sm' : 'group-hover/item:bg-slate-100 dark:group-hover/item:bg-slate-800'}
                    `}>
                        <item.icon 
                            size={22} 
                            strokeWidth={isActive ? 2.5 : 2}
                            className={`transition-transform duration-300 ${isActive ? 'scale-110' : 'group-hover/item:scale-110'}`} 
                        />
                    </div>
                </div>
        ...
        <div className="flex-shrink-0 border-t border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm">
           <div className="py-8 flex flex-col gap-4">
              <div className="flex items-center justify-center w-full relative h-10">
                 <div className={`flex gap-2 transition-all duration-300 ${isMobileMenuOpen ? 'gap-4' : 'md:group-hover:gap-3'}`}>
                    <a href="https://github.com/ekinkoseoglu" target="_blank" rel="noreferrer" 
                    ...
                    <a href="https://linkedin.com/in/ekinkoseoglu" target="_blank" rel="noreferrer" 
