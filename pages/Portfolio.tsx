...
import { MapPin, ExternalLink, Github, GraduationCap, Briefcase, ArrowRight } from 'lucide-react';
...
const PortfolioSection: React.FC = () => {
  ...
  return (
    <section id="portfolio" className="min-h-screen bg-slate-50 dark:bg-slate-950 py-24 transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <ScrollReveal>
            ...
            <h2 className="text-sm font-bold text-brand-600 dark:text-brand-400 tracking-widest uppercase mb-3">Portfolio</h2>
            <h3 className="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white mb-6">My Journey</h3>
            <p className="max-w-2xl mx-auto text-lg text-slate-600 dark:text-slate-400">
                A timeline of my professional career, education, and the open-source projects I've built.
            </p>
            </div>
        </ScrollReveal>
            ...
            <div className="lg:col-span-2 space-y-8">
                <ScrollReveal>
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center mb-8">
                        <Briefcase className="mr-3 text-brand-600 dark:text-brand-400" /> Work History
                    </h3>
                </ScrollReveal>
                    ...
                    <div className="relative pl-8 md:pl-12 group">
                        ...
                        <div className="relative z-10">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                                <div>
                                <h4 className="text-xl font-extrabold text-slate-900 dark:text-white">{exp.role}</h4>
                                <div className="text-brand-600 dark:text-brand-400 font-bold text-lg">{exp.company}</div>
                                </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                ...
                                <span key={skill} className="px-3 py-1 rounded-full text-xs font-semibold bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-300 border border-brand-100 dark:border-brand-800">
                                    {skill}
                                </span>
                                ))}
                            </div>
            ...
            <div>
              <div className="sticky top-24">
                <ScrollReveal delay={300}>
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center mb-8">
                        <GraduationCap className="mr-3 text-brand-600 dark:text-brand-400" /> Education
                    </h3>
                    <div className="space-y-6">
                    {EDUCATION.map((edu, index) => (
                        <div key={index} className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-lg transition-all duration-300 group">
                        <div className="text-brand-600 dark:text-brand-400 font-bold text-sm mb-2 opacity-80 group-hover:opacity-100">{edu.year}</div>
                        <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-2">{edu.school}</h4>
                        <p className="text-slate-600 dark:text-slate-400 leading-relaxed">{edu.degree}</p>
                        </div>
                    ))}
                    </div>
                </ScrollReveal>
            ...
            {PROJECTS.map((project, index) => (
              <ScrollReveal key={index} delay={index * 100}>
                <div className="bg-white dark:bg-slate-900 rounded-3xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-800 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col group h-full">
                    <div className="h-48 bg-gradient-to-br from-slate-800 to-slate-950 p-6 flex items-center justify-center relative overflow-hidden">
                        {/* Abstract Tech Patterns */}
                        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-brand-500/20 via-slate-900/0 to-transparent"></div>
                        <div className="absolute inset-0 bg-brand-600 opacity-0 group-hover:opacity-10 transition-opacity duration-500"></div>
                        
                        <h3 className="text-2xl font-bold text-white text-center z-10 transform group-hover:scale-105 transition-transform duration-300">{project.title}</h3>
                    ...
                    <div className="text-xs font-bold tracking-widest text-brand-600 dark:text-brand-400 uppercase mb-3">{project.period}</div>
                    ...
                    <div className="space-y-6 mt-auto">
                        ...
                        {project.techStack.slice(0, 4).map((tech) => (
                            <span key={tech} className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-xs rounded-md font-semibold border border-slate-200 dark:border-slate-700">
                            {tech}
                            </span>
                        ))}
